...
- (void)mergeAndExportVideosAtFileURLs:(NSArray *)fileURLArray
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        //重新构造
        CGFloat htemp=renderSize.width/VIDEO_SCALE;
        CGSize renderTemp=CGSizeMake(renderSize.width,htemp);
        
        
        //        CGFloat renderW = MIN(renderTemp.width, renderTemp.height);
        CGFloat renderW=renderTemp.width;
        for (int i = 0; i < [assetArray count] && i < [assetTrackArray count]; i++) {
            ...
            rate = renderW / assetTrack.naturalSize.width;
            
//            CGAffineTransform layerTransform = CGAffineTransformMake(assetTrack.preferredTransform.a, assetTrack.preferredTransform.b, assetTrack.preferredTransform.c, assetTrack.preferredTransform.d, assetTrack.preferredTransform.tx * rate, assetTrack.preferredTransform.ty * rate);
//            layerTransform = CGAffineTransformConcat(layerTransform, CGAffineTransformMake(1, 0, 0, 1, 0, -(assetTrack.naturalSize.width - assetTrack.naturalSize.height)));//向上移动取中部影响
...
//            [layerInstruciton setTransform:layerTransform atTime:kCMTimeZero];
        ...
        mainCompositionInst.renderSize = renderTemp;
...
- (void)encodeVideo:(NSURL *)videoURL
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSString *path = [documentsDirectory stringByAppendingPathComponent:@"video"];
        
        NSString *filename=[NSString stringWithFormat:@"%f.mp4",[NSDate timeIntervalSinceReferenceDate]];
        NSString *outfilePath=[NSString stringWithFormat:@"%@/%@",path,filename];
        
        NSURL *exportUrl = [NSURL fileURLWithPath:outfilePath];
        ...
        [exportSession exportAsynchronouslyWithCompletionHandler:^{
            switch ([exportSession status])
            {
                    ...
                    NSLog(@"MP4 Successful!");
                {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        if ([_delegate respondsToSelector:@selector(videoRecorder:didFinishMergingVideosToOutPutFileAtURL:)]) {
                            [_delegate videoRecorder:self didFinishMergingVideosToOutPutFileAtURL:exportUrl];
                        }
                    });
                }
                    break;
